...
assert.match(source, /window\.showUpcoming = function\(\)[\s\S]*?closeMorePanel\(\)[\s\S]*?showHome\(\)[\s\S]*?getElementById\('rowUpcoming'\)[\s\S]*?scrollIntoView\(\{ behavior: 'smooth', block: 'center' \}\)/);
